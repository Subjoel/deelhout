<div class="row">
    <div class="col-12">
                ...
                if (authCheck()):
                    if (!empty($product->digital_file_download_link)): ?>
                        <div class="row-custom">
                            <a href="<?= esc($product->digital_file_download_link); ?>" class="btn btn-instant-download" target="_blank"><i class="icon-download-solid"></i><?= trans("download") ?></a>
                        </div>
                    <?php else: ?>
                        <div class="row-custom">
        ...
        <?php 
        require_once $_SERVER['DOCUMENT_ROOT'] . '/app/Models/ProductAdminModel.php';
        if (date_create(date('m/d/Y', strtotime($product->created_at))) < date_create(date('m/d/Y', strtotime("1 month ago"))) && date_create(date('m/d/Y H:i:s', strtotime($product->created_at))) > date_create(date('m/d/Y H:i:s', strtotime("3 months ago")))): ?>
        <h1 class="product-title"><?= esc($title); ?> 🟡</h1>
        <?php elseif (date_create(date('m/d/Y', strtotime($product->created_at))) < date_create(date('m/d/Y', strtotime("-3 months")))): ?>
        <h1 class="product-title"><?= esc($title); ?> 🔴</h1>
        <?php else: ?>
        <h1 class="product-title"><?= esc($title); ?> 🟢</h1>
        <?php endif; ?>
